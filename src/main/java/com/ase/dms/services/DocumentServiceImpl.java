...
public class DocumentServiceImpl implements DocumentService {
  ...
  public DocumentServiceImpl() { // Initializing with a dummy document for testing purposes
    ...
    DocumentEntity dummyDocument = new DocumentEntity(id, "dummy.txt", "text/plain", 1024L, "test-id", "owner-id", LocalDateTime.now(), downloadUrl);
  ...
  public DocumentEntity createDocument(MultipartFile file, String folderId) {
    ...
    String downloadUrl = "/dms/v1/documents/" + id + "/download";
    String ownerId = "owner-id"; // TODO Here you should retrieve the actual owner ID
    DocumentEntity document = new DocumentEntity(id, file.getOriginalFilename(), file.getContentType(), file.getSize(), folderId, ownerId, LocalDateTime.now(), downloadUrl);
